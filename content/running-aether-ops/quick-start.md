...
This page walks through deploying a Kubernetes cluster and the 5G Core network on a single node using the Aether Ops. By the end you will have both components installed and running.

## Step 1: Run preflight checks

Before deploying anything, run the preflight checks to verify that the host meets all prerequisites. These checks verify that required tools are installed, SSH password authentication is enabled, and the `aether` OS user exists.

If you installed with `--preflight` (e.g. `install.sh --preflight --yes`), all fixable checks should already pass. You can skip ahead to verifying the results or proceed directly to Step 2.

<Tabs>
  <TabItem value="script" label="Setup Script" default>

If you did not use `--preflight` during installation, run the standalone management node setup script to fix all three fixable checks in one pass:

```bash
curl -fsSL https://raw.githubusercontent.com/aether-gui/aether-ops/main/scripts/setup-mgmt-node.sh | sudo bash -s -- --yes
```

This installs required packages (`git`, `make`, `ansible`, `openssh-server`, `iptables`), enables SSH password authentication, and creates the `aether` user with passwordless sudo. It is idempotent — running it again skips anything already configured.
...
Aether Ops enforces a single-task execution model. Only one deployment task can run at a time. If you attempt to start a second action while a task is already running, the API returns a `409 Conflict` response:
