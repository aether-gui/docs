...
This guide covers common problems and their solutions when operating aether-ops.
...
sudo journalctl -u aether-ops --no-pager -n 50
```
...
Either stop the conflicting process or start aether-ops on a different port:

```bash
aether-ops --listen 0.0.0.0:9090
```

### Permission errors

The data directory (default `/var/lib/aether-ops`) must be writable by the aether-ops process. Check ownership:

```bash
ls -la /var/lib/aether-ops
```
...
Something restarted the `aether-ops` service while a task was running. The most common trigger is Ubuntu's `needrestart` tool, which automatically restarts services after `apt` installs or upgrades packages. Since Aether OnRamp playbooks install packages (Docker, Python modules, etc.) via Ansible, `needrestart` detects that `aether-ops` is using updated libraries and restarts it mid-task.
...
```bash
journalctl -u aether-ops --no-pager -b | grep -E 'SIGTERM|Stop|Start'
```

Check if `needrestart` is installed and whether `aether-ops` is excluded:

```bash
dpkg -l needrestart
cat /etc/needrestart/conf.d/aether-ops.conf
```

### Fix

Create a `needrestart` exclusion so it never restarts `aether-ops`:
...
On startup, `aether-ops` automatically detects actions and deployments that were interrupted by the previous shutdown and marks them as failed. Re-submit the deployment to retry.
...
```bash
aether-ops --cors-origins http://localhost:5173
```
...
Ensure the aether-ops host can reach all nodes on the SSH port (default 22). The preflight `node-ssh-reachable` check can verify this:
...
{
  "engine": "sqlite",
  "path": "/var/lib/aether-ops/app.db",
...
If the repository is on the wrong branch or tag, restart aether-ops with the correct `--onramp-version` flag, or call the refresh endpoint after updating the flag.
...
```bash
rm -rf /var/lib/aether-ops/certs/
sudo systemctl restart aether-ops
```
...
```bash
curl --cacert /var/lib/aether-ops/certs/ca.pem https://localhost:8186/api/v1/meta/version
```
