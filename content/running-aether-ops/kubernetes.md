...
The repository includes Kubernetes manifests in `deploy/k8s/` for deploying aether-ops to a cluster.
...
```bash
kubectl get pods -l app=aether-ops
```

Check the service:

```bash
kubectl get svc aether-ops
```
...
```bash
kubectl logs -l app=aether-ops
```

Follow logs in real time:

```bash
kubectl logs -f -l app=aether-ops
```
...
```bash
kubectl create secret tls aether-ops-tls \
  --cert=cert.pem \
  --key=key.pem
```
...
containers:
  - name: aether-ops
    ...
    args:
      ...
      - "0.0.0.0:8186"
      - --tls-cert
      - /etc/aether-ops/tls/tls.crt
      - --tls-key
      - /etc/aether-ops/tls/tls.key
    volumeMounts:
      - name: tls-certs
        mountPath: /etc/aether-ops/tls
        readOnly: true
volumes:
  - name: tls-certs
    secret:
      secretName: aether-ops-tls
```
...
```bash
kubectl create secret generic aether-ops-token \
