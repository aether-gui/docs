...
The `ghcr.io/bengrewell/aether-ops:latest` Docker image packages aether-ops for container-based deployments.
...
docker run -d \
  --name aether-ops \
  -p 8186:8186 \
  ghcr.io/bengrewell/aether-ops:latest \
  --listen 0.0.0.0:8186
```
...
docker run -d \
  --name aether-ops \
  -p 8186:8186 \
  -v aether-data:/var/lib/aether-ops \
  ghcr.io/bengrewell/aether-ops:latest \
  --listen 0.0.0.0:8186
```
...
For auto-generated TLS certificates, the persistent data volume is sufficient -- certificates are written to `/var/lib/aether-ops/certs/` automatically:
...
```bash
docker exec aether-ops curl -sf http://localhost:8186/healthz
```
