...
This guide covers enabling TLS, mutual TLS (mTLS), and token authentication for aether-ops.
...
Or via the systemd environment file (`/etc/aether-ops/env`):

```
AETHER_TLS=true
```

On first run, this generates a local CA and CA-signed server certificate under `{data-dir}/certs/` (default: `/var/lib/aether-ops/certs/`). The generated files are reused on subsequent starts.
...
```bash
sudo cp /var/lib/aether-ops/certs/ca.pem /usr/local/share/ca-certificates/aether-ops.crt
...
To regenerate certificates (e.g., after expiry), delete the `certs/` directory and restart aether-ops.

## Enable TLS with your own certificates

```bash
aether-ops --tls-cert /path/to/cert.pem --tls-key /path/to/key.pem
```
...
```bash
aether-ops --tls --mtls-ca-cert /path/to/client-ca.pem
```
...
```bash
curl --cacert /var/lib/aether-ops/certs/ca.pem \
     --cert client.pem \
     --key client-key.pem \
...
```bash
aether-ops --api-token mysecrettoken
```
...
Via the systemd environment file (`/etc/aether-ops/env`):
...
```bash
aether-ops --cors-origins "http://localhost:5173,https://admin.example.com"
```

Use `'*'` to allow all origins (not recommended for production):

```bash
aether-ops --cors-origins '*'
```
...
- Set the token via the `AETHER_API_TOKEN` environment variable (or in `/etc/aether-ops/env` for systemd) to keep it out of process listings.
- Place aether-ops behind a reverse proxy (e.g., Caddy, nginx) for TLS termination in production.
