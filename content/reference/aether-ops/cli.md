...
`aether-ops` is the backend daemon for the Aether Ops. It serves the REST API, embedded frontend, and manages all deployment operations.
...
| Flag | Env Var | Description | Default |
|------|---------|-------------|---------|
| `--data-dir` | `AETHER_DATA_DIR` | Directory for persistent state database | `/var/lib/aether-ops` |
...
| Flag | Env Var | Description | Default |
|------|---------|-------------|---------|
| `--mcp` | `AETHER_MCP` | Enable MCP server on stdio transport | `false` |
| `--mcp-listen` | `AETHER_MCP_LISTEN` | Address for MCP StreamableHTTP transport (e.g., `0.0.0.0:8187`) | - |
...
| `AETHER_MCP` | Enable MCP server on stdio (`true`, `1`, `yes`) | `--mcp` |
| `AETHER_MCP_LISTEN` | Address for MCP StreamableHTTP transport | `--mcp-listen` |
...
```bash
aether-ops --listen 0.0.0.0:8186
```
...
```bash
aether-ops --tls-cert /etc/aether-ops/cert.pem --tls-key /etc/aether-ops/key.pem
```
...
```bash
aether-ops \
  --tls-cert /etc/aether-ops/cert.pem \
  --tls-key /etc/aether-ops/key.pem \
  --mtls-ca-cert /etc/aether-ops/ca.pem
```
...
AETHER_CORS_ORIGINS='*' aether-ops
```

### API-only mode (no frontend)

```bash
aether-ops --serve-frontend=false
```
