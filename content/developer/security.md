...
On first run, generates a local CA and CA-signed server certificate under `<data-dir>/certs/` (default: `/var/lib/aether-ops/certs/`). Certificates are persisted and reused on subsequent starts.
...
- Server CN: `aether-ops`, CA CN: `Aether Ops CA`
...
```bash
aether-ops --tls --mtls-ca-cert /path/to/ca.pem
```
...
```bash
aether-ops --tls --api-token dev-token-123
```
