...
Deploying network components takes time -- installing a Kubernetes cluster or a 5G core network can run for several minutes. Rather than blocking the HTTP request for the entire duration, Aether Ops executes deployment actions asynchronously using tasks.
